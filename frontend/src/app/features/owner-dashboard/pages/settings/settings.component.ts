...
export class SettingsComponent implements AfterViewInit, OnInit {
  ...
  isGalleryCropOpen = false;
  galleryCropFile: File | null = null;
  galleryCropIndex: number | null = null;
  ...
  onGalleryFileSelected(index: number, event: Event): void {
    ...
    try {
      ...
      this.galleryCropFile = file;
      this.galleryCropIndex = index;
      this.isGalleryCropOpen = true;
    } catch (error) {
  ...
  clearGalleryImage(index: number): void {
    ...
    this.galleryDraft.images[index].file = null;
  }

  closeGalleryCropper(): void {
    this.isGalleryCropOpen = false;
    this.galleryCropFile = null;
    this.galleryCropIndex = null;
  }

  async applyGalleryCrop(file: File): Promise<void> {
    if (this.galleryCropIndex === null) {
      this.closeGalleryCropper();
      return;
    }

    try {
      const processedImage = await this.imageUploadService.prepareImage(file);
      this.galleryDraft.images[this.galleryCropIndex].previewUrl = processedImage.previewUrl;
      this.galleryDraft.images[this.galleryCropIndex].fileName = processedImage.file.name;
      this.galleryDraft.images[this.galleryCropIndex].file = processedImage.file;
    } catch (error) {
      alert(error instanceof Error ? error.message : 'A kep feldolgozasa sikertelen.');
    } finally {
      this.closeGalleryCropper();
  ...
  private syncGalleryFromApiSources(): void {
    const defaultSlots: GalleryImage[] = [
      { slotName: 'Főkép', title: 'Főkép', fileName: '', previewUrl: null, file: null },
      { slotName: 'Kép 2', title: 'Kép 2', fileName: '', previewUrl: null, file: null },
      { slotName: 'Kép 3', title: 'Kép 3', fileName: '', previewUrl: null, file: null },
      { slotName: 'Kép 4', title: 'Kép 4', fileName: '', previewUrl: null, file: null },
    ];
